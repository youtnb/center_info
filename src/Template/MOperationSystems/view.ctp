...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit M Operation System'), ['action' => 'edit', $mOperationSystem->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete M Operation System'), ['action' => 'delete', $mOperationSystem->id], ['confirm' => __('Are you sure you want to delete # {0}?', $mOperationSystem->id)]) ?> </li>
...
<div class="mOperationSystems view large-9 medium-8 columns content">
    <h3><?= h($mOperationSystem->name) ?></h3>
    <table class="vertical-table">
        ...
        <tr>
            <th scope="row"><?= __('Server Flag') ?></th>
            <td><?= $mOperationSystem->server_flag ? __('Yes') : __('No'); ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Delete Flag') ?></th>
            <td><?= $mOperationSystem->delete_flag ? __('Yes') : __('No'); ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Devices') ?></h4>
        <?php if (!empty($mOperationSystem->devices)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($mOperationSystem->devices as $devices): ?>
            <tr>
                <td><?= h($devices->id) ?></td>
