...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit M Sqlserver'), ['action' => 'edit', $mSqlserver->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete M Sqlserver'), ['action' => 'delete', $mSqlserver->id], ['confirm' => __('Are you sure you want to delete # {0}?', $mSqlserver->id)]) ?> </li>
...
<div class="mSqlservers view large-9 medium-8 columns content">
    <h3><?= h($mSqlserver->name) ?></h3>
    <table class="vertical-table">
        ...
        <tr>
            ...
            <td><?= $mSqlserver->delete_flag ? __('Yes') : __('No'); ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Devices') ?></h4>
        <?php if (!empty($mSqlserver->devices)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($mSqlserver->devices as $devices): ?>
            <tr>
                <td><?= h($devices->id) ?></td>
