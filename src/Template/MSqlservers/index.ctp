 ...
 * @var \App\Model\Entity\MSqlserver[]|\Cake\Collection\CollectionInterface $mSqlservers
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    ...
    <ul class="side-nav">
        <li class="heading"><?= __('SQLServer種別マスタ') ?></li>
        <li><?= $this->Html->link(__('登録'), ['action' => 'add']) ?></li>
    </ul>
    <?php echo $this->element('navi_master', ['own' => 'MSqlservers']); ?>
</nav>
<div class="mSqlservers index large-9 medium-8 columns content">
    <h3><?= __('M Sqlservers') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($mSqlservers as $mSqlserver): ?>
            <tr>
                ...
                <td><?= h($mSqlserver->created) ?></td>
                <td><?= h($mSqlserver->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $mSqlserver->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $mSqlserver->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $mSqlserver->id], ['confirm' => __('Are you sure you want to delete # {0}?', $mSqlserver->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
